...
function wnoc(dtails){
	...
	if( ix>=0 && tbs[ix].disc===false && !tt && (ac_tab.cu === details.tabId || ac_tab.cu ===  tbs[ix].op_id )  && ( ( tbs[ix].op_url !== tbs[ix].og_url ) || ( tbs[ix].op_url!==tbs[ix].urls[0] ) ) && (tbs[ix].og_url!==tbs[ix].urls[0] && tbs[ix].urls.filter( (u)=>{return u!==tbs[ix].og_url;}).length>0  )  && !chr_tab && !tbs[ix].og_url.startsWith('about:') && !du.startsWith('about:blank') &&  tbs[ix].op_id!==-2){
